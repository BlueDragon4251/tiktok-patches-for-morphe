...
* Bottom-bar initialization and all native background writers in MainPageBusinessAssem,
  using the resolved background field and actual instruction registers.
...
Native lifecycle hooks now register MainPageFragment, both sidebar fragments,
AbstractSearchIntermediateFragmentNew and BaseChatRoomFragment roots. Only the registered
main page is compensated during native scrolling; no displaced intermediate value is
